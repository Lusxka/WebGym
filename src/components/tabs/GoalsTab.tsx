    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('dailyGoals')}</h1>
                            ...
                            <Card className="p-6">
                                <div className="flex items-center gap-4 mb-6">
                                    <div className={`p-3 rounded-lg bg-${goal.color}-500/20`}>
                                        <Icon size={24} className={`text-${goal.color}-600 dark:text-${goal.color}-400`} />
                                    </div>
                                    <div>
                                        {/* CORREÇÃO: Título com cores para os dois modos */}
            ...
            <Card className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {achievements.map(ach => (
                        <div key={ach.name} 
                            // CORREÇÃO: Adiciona classes de fundo para os dois modos
                            className={`p-4 rounded-lg text-center transition-opacity ${ach.unlocked 
                                ? 'bg-gray-100 dark:bg-gray-700/50' 
                                : 'bg-gray-100 dark:bg-gray-700/30 opacity-50'}`}
                        >
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 ${ach.unlocked ? 'bg-yellow-500/20' : 'bg-gray-200 dark:bg-gray-600'}`}>
                                <Award size={24} className={ach.unlocked ? 'text-yellow-500 dark:text-yellow-400' : 'text-gray-500'} />
                            </div>
                            {/* CORREÇÃO: Título e descrição com cores para os dois modos */}
                            <h3 className={`font-medium mb-1 ${ach.unlocked ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}>{ach.name}</h3>
                            <p className={`text-sm ${ach.unlocked ? 'text-gray-600 dark:text-gray-400' : 'text-gray-500'}`}>{ach.description}</p>
                        </div>
                    ))}
                </div>
            </Card>
            ...
            <Card className="p-6">
                {/* CORREÇÃO: Título com cores para os dois modos */}
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Visão Geral Semanal</h3>
                <div className="space-y-3">
                    {state.weeklyProgress.map((dayData) => (
                        <div key={dayData.day} 
                            ...
                            {/* CORREÇÃO: Texto com cores para os dois modos */}
                            <span className="text-gray-600 dark:text-gray-300">{dayData.day}</span>
